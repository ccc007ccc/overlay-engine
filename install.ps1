...
function Invoke-ReleaseInstall {
    ...
    if (Test-Component 'DesktopMonitor') {
        Set-Content -Path (Join-Path $InstallDir 'config.ini') -Encoding UTF8 -Value @('# Generated by overlay-engine installer.', 'Launch=desktop-window-monitor.exe')
    } else {
        Set-Content -Path (Join-Path $InstallDir 'config.ini') -Encoding UTF8 -Value @('# Generated by overlay-engine installer.')
    }
