...
色块、动态橙色块)留在 World 空间。直接启动 `demo-app` 会默认请求 3 个 Desktop monitor，拖到屏幕不同位置后盯两件事:
...
### 终端 2 — demo-app 默认请求三个 Desktop monitor
...
3. 通过 `StartMonitor` 请求 3 个 Desktop monitor，并等待 `StartMonitorResult` 返回实际 monitor id。
4. 之后 3 个 Desktop monitor 窗口应该开始显示内容。
