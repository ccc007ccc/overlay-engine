...
3. `Launch=desktop-window-monitor.exe` 自动拉起 desktop monitor，不需要用户单独启动 monitor。
4. 卸载后 `core-server.exe`、`desktop-window-monitor.exe`、快捷方式、开始菜单目录和安装目录都被删除。
...
2. 注销/重新登录后 core-server 自动启动，工作目录仍是安装目录。
3. `config.ini` 生效，desktop monitor 自动拉起。
